...
class search_direct:
    ...
    def search(self,books_by_name,books_by_author):
        ...
        if(none_flag):
            ...
            return str(result_by_author.to_html())
        elif(len(books_by_author)==0):
            return str(result_by_name.to_html())
        combined=pd.merge(result_by_author,result_by_name, how='inner', on=['BookName'])
        if(len(combined)==0):
            frames = [result_by_author,result_by_name]
            result = pd.concat(frames, keys=['From_Author', 'From_title'])
            ...
            return str(combined.iloc[:,:len(self.books_df.columns)].to_html())
